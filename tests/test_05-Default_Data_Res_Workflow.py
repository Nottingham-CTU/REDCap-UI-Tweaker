...
# Test name: t05 - Default Data Res. Workflow
import pytest
import time
import json
...
class Test_05_Default_Data_Res_Workflow:
  ...
  def test_05_Default_Data_Res_Workflow(self):
    self.driver.get("http://127.0.0.1/")
    self.driver.find_element(By.LINK_TEXT, "My Projects").click()
    assert len(self.driver.find_elements(By.XPATH, "//*[@id=\"table-proj_table\"][contains(.,'REDCap UI Tweaker Test')]")) > 0
    assert len(self.driver.find_elements(By.XPATH, "//*[@id=\"table-proj_table\"][contains(.,'REDCap UI Tweaker DRW Test')]")) == 0
    ...
    None if (element := self.driver.find_element(By.CSS_SELECTOR, "[name=\"data-res-workflow\"]")).is_selected() else element.click()
    ...
    assert self.driver.find_element(By.ID, "data_resolution_enabled").get_attribute("value") == "2"
