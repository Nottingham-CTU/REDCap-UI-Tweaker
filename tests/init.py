...
class Test_init:
  ...
  def test_init(self):
    ...
    assert len(self.driver.find_elements(By.CSS_SELECTOR, "#external-modules-enabled tr[data-module=\"redcap_ui_tweaker\"]")) > 0
    ...
    time.sleep(0.5)
    self.driver.find_element(By.ID, "user_role").find_element(By.XPATH, "(descendant::option)[. = 'TestRole']").click()
    ...
    time.sleep(3)
    self.driver.find_element(By.ID, "user_role").find_element(By.XPATH, "(descendant::option)[. = 'TestRole2']").click()
    None if not (element := self.driver.find_element(By.CSS_SELECTOR, "input[name=\"notify_email_role\"]")).is_selected() else element.click()
    self.driver.find_element(By.ID, "assignDagRoleBtn").click()
    time.sleep(3)
