...
# Test name: t04 - SQLDESCRIPTIVE action tag
import pytest
import time
import json
...
class Test_04_SQLDESCRIPTIVE_action_tag:
  ...
  def test_04_SQLDESCRIPTIVE_action_tag(self):
    ...
    None if (element := self.driver.find_element(By.CSS_SELECTOR, "[name=\"sql-descriptive\"]")).is_selected() else element.click()
    ...
    self.driver.find_element(By.ID, "field_type").find_element(By.CSS_SELECTOR, "*[value='sql']").click()
    ...
    assert len(self.driver.find_elements(By.CSS_SELECTOR, "body[data-hassqldesc]")) > 0
    self.driver.execute_script("window.dataEntryFormValuesChanged=false")
    self.driver.find_element(By.CSS_SELECTOR, "a[href*=\"online_designer.php\"]").click()
    self.driver.find_element(By.LINK_TEXT, "Basic Demography Form").click()
    self.driver.execute_script("$('#design-sql2 a[onclick*=\"deleteField\"]').remove()")
